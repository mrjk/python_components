...
def test_get_layer(varmgr):
    """get_layer returns a Layer object, not the raw payload."""
    layer = varmgr.get_layer("app_cli")
    assert isinstance(layer, Layer)
    assert layer.source.name == "app_cli"
    assert layer.payload["name"] == "dataset1"
...
def test_add_duplicate_source():
    ...
    assert varmgr._sources["test_source"].level == 200


def test_add_duplicate_source_list():
    """List add_sources must honor force the same way as single-source."""
    varmgr = StoreManager()
    varmgr.add_sources([Source("test_source", level=100)])
